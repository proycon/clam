...
if __name__ == "__main__":
    ...
    MTSYSTEM_NLFY = sys.argv[4]
    MTSYSTEM_FYNL = sys.argv[5]
    ...
    for i, inputfile in enumerate(clamdata.input):
        ...
        if str(inputfile)[-7:] == '.nl.txt':            
            r = os.system(MTSYSTEM_NLFY + ' run ' + str(inputfile) + ' > ' + outputdir + '/' + os.path.basename(inputfile)[:-7] + '.fy.txt')
        elif str(inputfile)[-7:] == '.fy.txt':
            r = os.system(MTSYSTEM_FYNL + ' run ' + str(inputfile) + ' > ' + outputdir + '/' + os.path.basename(inputfile)[:-7] + '.nl.txt')        
