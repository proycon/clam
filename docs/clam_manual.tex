...
{\bf \small version 0.9.12 - revision 1.1} \\[0.5cm]
...
and more difficult to install. Windows is not supported.
...
{ \small
\begin{verbatim} $ sudo easy_install -U clam \end{verbatim}
}

It it wise to repeatedly issue this command (every month or so), as
updates are released on a regular basis.

In case \texttt{easy\_install} is not yet available on your system, you first need to
install it. On a Debian/Ubuntu based distribution this can be done as follows: 
...
flag, but this forces the user to manually ensure the target directory is part
...
To install CLAM globally or in your local Python packages, you can use the
...
Especially people migrating from earlier versions of CLAM may already have
...
desired. The path \path{/path/to/clam/static/} should be changed to where CLAM is
installed and where the \path{static} directory is found. Depending on your
installation and versions, this will be a directory like: \\
\path{/usr/local/lib/python2.7/dist-packages/CLAM-0.9.8.3-py2.7.egg/clam/static}
...
\item Configure your service configuration file as explained in
  Section~\ref{sec:serviceconfig}. Take special note of Subsection~\ref{sec:sadmin} where you are instructed to configure the hostname, port, and optionally a URL prefix to use if the service is not assigned a virtualhost of its own.
...
\item Add and adapt the following configuration to a server in \\ \path{/etc/nginx/sites-enabled} . Note that in this example we assume that \texttt{URLPREFIX} in the service configuration file is set to an empty string (or not set at all), effectively exposing CLAM at the root of the server. You may configure a \texttt{URLPREFIX} when desired. In that case, take care to update the below \texttt{location} and directives accordingly:
{ \small
\begin{verbatim}
 root /path/to/clam;
...
\textbf{Querystring} &  Accepted parameters are defined in the Service Configuration file (and thus differ per service). The parameter ID corresponds to the parameter keys in the querystring \\
...
\textbf{Querystring} & The parameter \texttt{abortonly} can be set to 1 if you
...
\textbf{Description} & This method adds a new input file. Response is returned
in CLAM-Upload XML (distinct from CLAM XML!). Two arguments are mandatory: the
...
a metadata field as parameter to the querystring, with the same ID as defined
...
\textbf{Querystring} & Determined by the action  \\
