...
{ \small
\begin{verbatim}
uwsgi --socket :3031 --chdir /path/to/yourwebservice --wsgi-file /path/to/yourwebservice/yourwebservice.wsgi --master --processes 4 --threads 2
\end{verbatim}
}
...
Alternatively, you can use UNIX sockets too: \texttt{--socket
/tmp/yourwebservice.sock}, which is slightly faster even.
...
\item Add and adapt the following configuration to a server in \\
    \path{/etc/nginx/sites-enabled}. If you use expose the application at the
