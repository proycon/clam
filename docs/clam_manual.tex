...
responsible for authenticating you, your user credentials. Many OAuth2
providers exists; such as Google, Facebook and Github, but you most likely want
to use the OAuth2 provider of your own institution. You will need to register
your webservice with your authentication provider, and obtain a
\texttt{CLIENT_ID} and \texttt{CLIENT_SECRET}, the latter should be kept
strictly private. These go into your service configuration file and we then
enable OAuth as follows: 
...
authentication provider\footnote{CLAM responds with a HTTP 303 - See Other}.
The authentication provider in turn redirects the user back to the CLAM
...
Passing the access token can be done in two ways, the recommended way is by
sending the following HTTP header in your request, where the number is replaced
...
def myprovider_username_function(oauthsession):
  r = oauthsession.get("https://myprovider/user")
...
done using the \texttt{OAUTH_SCOPE} directive in the service configuration file, which
takes a list of scopes, all of which are provider-specific. The following
...
goes to CLAM to obtain the actual access token.
...
return \texttt{HTTP 303 - See Other} and redirect to the authentication provider.
