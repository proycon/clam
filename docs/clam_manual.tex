...
The CLAM webservice is a RESTful webservice, meaning it uses the HTTP verbs
...
responds in the CLAM XML format. An associated XSL stylesheet can directly
transform this to xhtml in the user's browser, thus providing a standalone web
application for humand end-users. 
...
CLAM is written in Python 2.6, and is built on the webpy framework. It can run
...
implements HTTP Digest Authentication, which as opposed to HTTP Basic
...
handles things such as reading the \texttt{clam.xml} data file, and makes all
parameters and input files (with metadata) directly accessible.
